...
A \term{phonetic transcription} is a writing system that represents every speech sound distinctly and unambiguously from all others.
...
A \term{root} is a morpheme that determines the core meaning of a word.
In many languages a root can occur alone (free) as a word.
For example in English the verb \fm{paint} is a word based on the root \fm{\rt{paint}} which can also be found in the derived noun \fm{painter};%
...
similarly the verb \fm{read} is a word based on the root \fm{\rt{read}} that can also be found in the derived verb \fm{reread} \parencite[42]{fromkin-rodman-hyams:2014}.
But not all roots even in English are free; for example in the word \fm{jubilation} there is a root \fm{\rt{jubil}} which cannot occur alone \parencite[66]{matthews:1991}, and similarly the root \fm{\rt{kemp}} in \fm{unkempt} or \fm{\rt{cern}} in \fm{discern} also cannot be used alone as words \parencite[43]{fromkin-rodman-hyams:2014}.
A root that never occurs free is called a \term{bound root} because it is always bound to some other morphemes.
...
For example, the Arabic root \fm{\rt{ktb}} ‘writing’ gives rise to words like \fm{\gm{k}i\gm{t}ā\gm{b}} ‘book’, \fm{\gm{k}u\gm{t}ayyi\gm{b}} ‘booklet’, \fm{isti\gm{k}\gm{t}ā\gm{b}} ‘dictation’, and \fm{ma\gm{k}\gm{t}a\gm{b}} ‘desk, office’.
Because the root \fm{\rt{ktb}} does not contain any vowels it cannot be pronounced as a word in Arabic, and so it can never occur free.
For similar reasons detailed in chapter \ref{ch:root+stem}, all Tlingit verb roots are bound roots.

An \term{affix} is a morpheme that always occurs attached to some other morphological unit so that they form a phonological unit \parencite[9]{booij:2007}.
...
A typical example of a prefix in English is \fm{un-} which can be added to many verbs to indicate removal or release from the situation described by the verb.
...
A typical example of a suffix in English is \fm{-ed} which is added to most verbs to indicate past tense.
...
The root \fm{\rt{wrap}} in (\ref{ex:intro-ling-morph-pfxsfx-eng-free}) occurs free as a present tense verb form.
In (\ref{ex:intro-ling-morph-pfxsfx-eng-pfx}) the root is prefixed with \fm{un-}, and then in (\ref{ex:intro-ling-morph-pfxsfx-eng-sfx}) the root is suffixed with \fm{-ed}.
...
\exrtcmt{free root in English}%
\begingl
	\gla	I wrap it. //
	\glb	I \rt{wrap} it //
\endgl
\a\label{ex:intro-ling-morph-pfxsfx-eng-pfx}%
\exrtcmt{prefix in English}%
\begingl
	\gla	I \rlap{\gm{un}wrap} @ {} it. //
	\glb	I \gm{un}- \rt{wrap} it //
\endgl
\a\label{ex:intro-ling-morph-pfxsfx-eng-sfx}%
\exrtcmt{suffix in English}%
\begingl
	\gla	I \rlap{wrapp\gm{ed}} @ {} it. //
	\glb	I \rt{wrap} -\gm{ed} it //
\endgl
\a\label{ex:intro-ling-morph-pfxsfx-eng-pfx+sfx}%
\exrtcmt{prefix and suffix in English}%
\begingl
	\gla	I \rlap{\gm{un}wrapp\gm{ed}} @ {} @ {} it. //
	\glb	I \gm{un}- \rt{wrap} -\gm{ed} it //
\endgl
\xe

Although prefixes and suffixes are often shown in a linear order like beads on a string, this is not the whole story.
\FIXME{Hierarchy and semantic interpretation.}
