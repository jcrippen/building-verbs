...
The discussion of Tlingit grammar throughout this book depends on a variety of basic linguistic concepts. This section provides a rough overview of some linguistic topics and a summary of concepts used in the rest of this book. The concepts and definitions in this section are not meant to replace an introductory linguistics textbook \parencites[e.g.][]{radford-atkinson-britain-etc:2009}{yule:2010}{anderson:2018}, and readers are encouraged to look up definitions in other sources.
...
A \term{simple} onset or coda is one that contains only a single consonant. The word \fm{g̱áx̱} [\ipa{qáχ}] ‘rabbit’ in (\ref{ex:intro-ling-phon-syllable}) is a single syllable with a simple onset and a simple coda. When an onset or coda contains more than one consonant it is called \term{complex}. The classic example of a complex onset and a complex coda in English is the monosyllabic word \fm{strengths} [\ipa{stɹɛŋkθs}]. This syllable has an onset of three consonants [\ipa{s}], [\ipa{t}], and [\ipa{ɹ}] and a coda of four consonants [\ipa{ŋ}], [\ipa{k}], [\ipa{θ}], and [\ipa{s}]. A similar example in Tlingit is \fm{chx̱ánkʼ} [\ipa{tʃʰχánkʼ}] ‘grandchild’ shown in (\ref{ex:intro-ling-phon-syllable-grandchild}). This syllable  has an onset with an aspirated postalveolar affricate [\ipa{tʃʰ}] and a uvular fricative [\ipa{χ}] and a coda with a postalveolar nasal [\ipa{n}] and an ejective velar stop [\ipa{kʼ}].
...
Across all human languages a syllable must at least have a nucleus but need not have either an onset or a coda. An example of an onsetless syllable can be found in Hawaiian where the word \fm{ō} [\ipa{oː}] ‘answer’ contrasts with the word \fm{ʻō} [\ipa{ʔoː}] ‘pierce’ that has a glottal stop in its onset. Tlingit requires all syllables to have an onset; a word like \fm{áa} [\ipa{ʔáː}] ‘lake’ that is written without an initial consonant actually includes an unwritten onset glottal stop. All languages allow syllables without codas, and some – e.g.\ Hawaiian and Swahili – actually prohibit codas entirely. The term \term{open syllable} refers to a syllable that does not have a coda. The idea is that the syllable ends with the vocal tract open from producing the vowel. The words \fm{sháa} [\ipa{ʃáː}] ‘woman’ and \fm{shá} [\ipa{ʃá}] ‘head’ are open syllables. The term \term{closed syllable} refers to a syllable that does have a coda, with the idea that the syllable includes some level of closure in the vocal tract. The words \fm{g̱áx̱} [\ipa{qáχ}] ‘rabbit’ in (\ref{ex:intro-ling-phon-syllable}) and \fm{chx̱ánkʼ} [\ipa{tʃʰχánkʼ}] ‘grandchild’ in (\ref{ex:intro-ling-phon-syllable-grandchild}) are both closed syllables.
...
The representation of speech with a transcription system is a technical practice separate from other forms of writing. Languages are more often written with an \term{orthography} which is a writing system and some associated conventions involving spelling, capitalization, word-breaking, and punctuation. The orthography for Tlingit is roughly phonemic in that a single symbol like \fm{sh} or \fm{x̱ʼ} generally represents a single sound.\footnote{Note that ‘letter’ ≠ ‘symbol’. The symbol \fm{sh} has two letters \fm{s} and \fm{h} that together represent a single phoneme /\ipa{ʃ}/.} But there are a few exceptions where the orthography can be misleading because it uses more than one symbol for the same sound or lacks a distinction between sounds. Three such exceptions are detailed in the following paragraphs.

One exception is that unaspirated stops and affricates are represented with different symbols depending on whether they are in the onset or coda. This is reflected in table \ref{tab:intro-ling-phon-consonants} that lists the consonant phonemes in Tlingit. Most consonants in table \ref{tab:intro-ling-phon-consonants} have a single orthographic symbol, but the unaspirated stops like /\ipa{k}/ and the unaspirated affricates like /\ipa{tʃ}/ have two orthographic symbols separated by a tilde \~, e.g.\ \fm{g\~k} and \fm{j\~ch}. The symbol in front of the tilde is used when the consonant occurs in the onset and the symbol after the tilde is used when the consonant is in the coda. The word \fm{káak} [\ipa{kʰáːk}] ‘maternal uncle’ has the same symbol \fm{k} at its beginning and the end, but the IPA transcription shows that the onset consonant is an aspirated velar stop [\ipa{kʰ}] where the coda consonant is an unaspirated velar stop [\ipa{k}]. Likewise, the word \fm{tsáats} [\ipa{tsʰáːts}] ‘bear root’ starts and ends with \fm{ts} symbols but the transcription has an aspirated alveolar affricate [\ipa{tsʰ}] in the onset and an unaspirated alveolar affricate [\ipa{ts}] in the coda. Tlingit actually prohibits aspirated stops and affricates from appearing in the coda so that a word like *[\ipa{kʰáːkʰ}] or *[\ipa{tsʰátsʰ}] is impossible \parencite{maddieson:2001}. Although the orthographic representation of coda consonants is potentially misleading, it is still unambiguous.

Another exception is that vowels in the Tlingit orthography always include implied tone and length. Using the IPA we can refer to a vowel like [\ipa{a}] which has neither tone or length indicated, distinguishing this from e.g.\ short high [\ipa{á}], short low [\ipa{à}], long high [\ipa{áː}], and long low [\ipa{àː}]. The orthography only provides four symbols \fm{á}, \fm{a}, \fm{áa}, and \fm{aa} so there is no equivalent to the IPA symbol [\ipa{a}]. It is impossible to pronounce a vowel in Tlingit without tone and length so this absence of an orthographic equivalent to [\ipa{a}] is not a problem for representing actual Tlingit language forms. But the orthographic representation of abstract elements which lack tone and length is problematic. Thus a root like \fm{\rt[¹]{gut}} ‘sg.\ go’ must be understood as representing [\ipa{kʷut}] without tone and length rather than the implied [\ipa{kʷùt}] with a short vowel and low tone.
